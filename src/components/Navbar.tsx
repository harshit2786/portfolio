...
import { TRANSLATIONS, type Lang, type Theme } from '../data';
...
const NAV_ITEMS = [
  { id: 'about',    href: '#about' },
  { id: 'exp',      href: '#experience' },
  { id: 'projects', href: '#projects' },
  { id: 'stack',    href: '#stack' },
  { id: 'contact',  href: '#contact' },
] as const;

export default function Navbar({ theme, lang, onThemeToggle, onLangToggle }: Props) {
  ...
  const labels: Record<string, string> = {
    about: t.about, exp: t.exp, projects: t.projects, stack: t.stack, contact: t.contact,
  };
    ...
    <>
      ...
      >
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          style={{ background: 'none', border: 'none', display: 'flex', alignItems: 'center', gap: 10, cursor: 'none' }}
        >
          <span style={{
            width: 32, height: 32,
          ...
          {NAV_ITEMS.map(item => (
            <button
              ...
              style={{
                ...
                color: active === item.id.replace('exp', 'experience') ? 'var(--primary)' : 'var(--text-m)',
                cursor: 'none',
                position: 'relative',
                padding: '4px 0',
            ...
            >
              {labels[item.id]}
              {active === item.id.replace('exp', 'experience') && (
                <motion.span
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <motion.button
            ...
            style={{
              background: 'var(--bg-card)', border: '1px solid var(--border-b)',
              borderRadius: 4, padding: 8, cursor: 'none',
              color: 'var(--text-m)', display: 'flex', alignItems: 'center',
              transition: 'all 0.3s',
            }}
          >
            <AnimatePresence mode="wait">
